...
public class TableColumnAdjuster implements PropertyChangeListener, TableModelListener {
	...
	private boolean bAdjustPercentage;
	...
	public void adjustColumns() {
		...
		int iTotalWidth = 0;
		if (bAdjustPercentage) {
			for (int i = 0; i < tcm.getColumnCount(); i++) {
				iTotalWidth += table.getColumnModel().getColumn(i).getWidth();
			}
		}
		System.out.println("TCA1 : " + iTotalWidth);
		for (int i = 0; i < tcm.getColumnCount(); i++) {
			adjustColumn(i);
		}
		...
		if (bAdjustPercentage) {
			int iNewWidth = 0;
			for (int i = 0; i < tcm.getColumnCount(); i++) {
				iNewWidth += table.getColumnModel().getColumn(i).getWidth();
			}
			if (iNewWidth > 0) {
				float fRatio = (float) iTotalWidth / (float) iNewWidth;
				for (int i = 0; i < tcm.getColumnCount() - 1; i++) {
					TableColumn tc = table.getColumnModel().getColumn(i);
					updateTableColumn(i, (int) (tc.getWidth() * fRatio));
					//tc.setWidth((int) (tc.getWidth() * fRatio));
					iTotalWidth -= tc.getWidth()+spacing;
				}
				// Adjust last so the total width do not change at all
				if (tcm.getColumnCount() > 0) {
					int i = tcm.getColumnCount() - 1;
					updateTableColumn(i, iTotalWidth);
				}
				iTotalWidth = 0;
				for (int i = 0; i < tcm.getColumnCount(); i++) {
					iTotalWidth += table.getColumnModel().getColumn(i).getWidth();
				}
				System.out.println("TCA2 : " + iTotalWidth);
			}
		}
	}

	public void setAdjustPercentage(boolean b) {
		bAdjustPercentage = b;
	}
	/*
